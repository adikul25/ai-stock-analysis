...
def yf_fundamental_analysis(ticker: str):
    ...
    try:
        current_ratio = balance_sheet.loc['Current Assets'].iloc[1] / balance_sheet.loc['Current Liabilities'].iloc[1]
        debt_to_equity = balance_sheet.loc['Total Liabilities Net Minority Interest'].iloc[1] / balance_sheet.loc['Stockholders Equity'].iloc[1]
        roe = financials.loc['Net Income'].iloc[1] / balance_sheet.loc['Stockholders Equity'].iloc[1]
        ...
        fcf = cash_flow.loc['Operating Cash Flow'].iloc[1] - cash_flow.loc['Capital Expenditure'].iloc[1]
    except:
