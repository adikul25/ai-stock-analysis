...
def yf_tech_analysis(ticker: str, period: str = "1y"):
    ...
    # Check if df is empty after add_all_ta_features
    if df.empty:
        return {"ticker": ticker, "error": "Dataframe is empty after adding technical analysis features. Please check the input data."}

    # Check if df is empty after dropna
    if df.empty:
        return {"ticker": ticker, "error": "Dataframe is empty after dropping null values. Please check the input data or adjust the period."}
